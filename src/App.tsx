...
export default function App() {
  ...
  // --- UNITED STATES GEOTARGETING & STRICT BOT BLOCKER MIDDLEWARE ---
  const [securityStatus, setSecurityStatus] = useState<'checking' | 'passed' | 'failed' | 'geo_restricted'>('checking');
  ...
  const [mathNumbers, setMathNumbers] = useState({ n1: 0, n2: 0 });
  const [userMathAnswer, setUserMathAnswer] = useState('');
  const [challengeAttempts, setChallengeAttempts] = useState(0);
  const [challengeErrorText, setChallengeErrorText] = useState('');
  const [trackingScore, setTrackingScore] = useState(0); // Mouse, stroke tracking score
  const [honeypotInput, setHoneypotInput] = useState('');

  // Generate numbers for interactive human mathematical proof
  useEffect(() => {
    setMathNumbers({
  ...
  // Monitor general organic human behavior (mouse drift, keystroke interactions, screen scrolling, touch inputs)
  useEffect(() => {
    const logUserActivity = () => {
      setTrackingScore(prev => Math.min(prev + 1, 10));
    };
    window.addEventListener('mousemove', logUserActivity, { passive: true });
    window.addEventListener('keydown', logUserActivity, { passive: true });
    window.addEventListener('scroll', logUserActivity, { passive: true });
    window.addEventListener('touchstart', logUserActivity, { passive: true });

    return () => {
      window.removeEventListener('mousemove', logUserActivity);
      window.removeEventListener('keydown', logUserActivity);
      window.removeEventListener('scroll', logUserActivity);
      window.removeEventListener('touchstart', logUserActivity);
    };
  }, []);

  // Core Security scanning routine triggered on paint
  useEffect(() => {
    let activeScan = true;
    const executeThreatCheck = async () => {
      // 1. Instantly whitelist legitimate web crawler agents (safeguards high Google/Bing indexes and organic page metrics)
      const isSearchCrawler = /Googlebot|Bingbot|Slurp|DuckDuckBot|Baiduspider|YandexBot|Sogou|AdSense|Pagead|AdsBot|Mediapartners|Lighthouse|HeadlessChrome/i.test(navigator.userAgent);
      if (isSearchCrawler) {
        if (activeScan) setSecurityStatus('passed');
        return;
      }

      const detectedThreats: string[] = [];

      // Threat check: navigator.webdriver controller flag (Selenium, puppeteer, node-scraper loops)
      if (navigator.webdriver) {
        detectedThreats.push("Active automated browser driver system flag (navigator.webdriver = true)");
      }

      // Threat check: Clean language and viewport definitions that headless environments omit
      const isHeadlessAgent = !navigator.languages || navigator.languages.length === 0 || /HeadlessChrome|jsdom/i.test(navigator.userAgent);
      if (isHeadlessAgent) {
        detectedThreats.push("Headless environment execution criteria recognized");
      }

      // Threat check: Global window scripts that persist during automated automation testing
      const knownBotHooks = ['__phantomgjs', '_phantom', 'callPhantom', 'spawn', '__nightmare', '_selenium', 'webdriver', 'domAutomation', 'Cypress'];
      knownBotHooks.forEach(hook => {
        if (hook in window) {
          detectedThreats.push(`Scraper injection library properties detected: window.${hook}`);
        }
      });

      // Threat check: Target mismatched platform screen variables
      if (navigator.maxTouchPoints === 0 && !('ontouchstart' in window) && navigator.platform === 'iPhone') {
        detectedThreats.push("Incoherent hardware components (iPhone device without touch support triggers)");
      }

      // Geo Geolocation Verification targeting United States ONLY
      try {
        const response = await fetch('https://ipapi.co/json/');
        if (!response.ok) throw new Error("API Limit Triggered");
        
        const ipInfo = await response.json();
        const countryCode = ipInfo.country_code; // e.g., "US", "CA", "GB", etc.
        const countryName = ipInfo.country_name || ipInfo.country || 'International Coordinate';
        const regionName = ipInfo.region || 'Unmapped Coordinate';

        if (!activeScan) return;
        setGeoCountry(countryName);
        setGeoState(regionName);

        if (countryCode && countryCode !== 'US') {
          // If NOT from US, restrict the raw landing page display to protect resources against commercial foreign clicks and competitors
          // Allows them to verify through standard math puzzle if they are genuine human US expats or travelers
          setSecurityStatus('geo_restricted');
        } else {
          // US user: run standard background deep bot block check
          if (detectedThreats.length > 0) {
            setThreatDetails(detectedThreats);
        ...
        // Fallback policy: if IP lookup fails due to client firewall/ad-blocker restriction, protect user experience.
        // Grant entrance contingent on standard automated system test pass, avoiding locks.
        if (!activeScan) return;
        if (detectedThreats.length > 0) {
          setThreatDetails(detectedThreats);
          setSecurityStatus('failed');
  ...
  // Secure interactive humanness verification
  const handleVerifySubmission = (e: React.FormEvent) => {
    e.preventDefault();
    
    // Honeypot logic check: if a script filled the hidden security input, deny bypass access
    if (honeypotInput.trim() !== '') {
      setChallengeErrorText("Automated payload rejection. Block code (BP-HTD-9).");
      return;
    }

    const correctMathValue = mathNumbers.n1 + mathNumbers.n2;
    if (parseInt(userMathAnswer.trim(), 10) === correctMathValue) {
      setSecurityStatus('passed');
    } else {
      setChallengeAttempts(prev => prev + 1);
      setChallengeErrorText("Calculated puzzle is incorrect. Verify the sum to request authorized porch delivery.");
    }
  };
  ...
  // Geotargeted & Active Anti-Bot Render Decisions
  if (securityStatus === 'checking') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans select-none">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/50 text-center space-y-6">
          ...
          <div className="space-y-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-800 rounded-full text-xs font-bold uppercase tracking-wider">
              <Activity className="w-3 h-3 animate-pulse" /> Security Decoupler Establishing
            </span>
            <h1 className="text-2xl font-black text-brand-navy tracking-tight uppercase">US Delivery Gate</h1>
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              Scanning local routing coordinates and verifying human security profiles to access verified direct dispatch pricing...
            </p>
          </div>
          <div className="pt-4 border-t border-slate-100 space-y-3 text-xs font-mono text-slate-400">
            <div className="flex justify-between">
              <span>Dynamic Shield:</span>
              <span className="text-slate-600 font-bold">SHIELD_SYS_PRO_v4</span>
            </div>
            <div className="flex justify-between">
              <span>Postal Geo IP Node:</span>
              <span className="text-blue-600 font-bold animate-pulse">ESTABLISHING</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (securityStatus === 'geo_restricted') {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 font-sans text-white">
        <div className="max-w-lg w-full bg-slate-950 rounded-[40px] p-8 md:p-10 border border-white/10 shadow-2xl space-y-8">
          <div className="space-y-4 text-center">
            <div className="mx-auto w-16 h-16 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl flex items-center justify-center">
            ...
            <div className="space-y-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded-full text-xs font-bold uppercase tracking-wider">
                📍 United States Shipping Scope Gate
              </span>
              <h1 className="text-3xl md:text-4xl font-black tracking-tight uppercase leading-none mt-2">
                United States Delivery Regions Only
              </h1>
              <p className="text-sm md:text-base text-slate-400 font-medium leading-relaxed max-w-sm mx-auto">
                Authentication records indicate you are browsing from <strong className="text-slate-200">{geoCountry || 'Outside USA'}</strong> ({geoState || 'International'}). Authentic direct factory prices are calibrated for US mailing lines only.
              </p>
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-3xl p-6 border border-white/5 space-y-4">
            <div className="flex gap-4 items-start text-sm">
              ...
              <div>
                <h4 className="font-bold text-slate-200">Are you a US Resident traveling abroad?</h4>
                <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                  To bypass the geographical security filter, complete our quick mathematical code puzzle to certify human access.
                </p>
              </div>
            </div>

            <form onSubmit={handleVerifySubmission} className="pt-4 border-t border-slate-850 space-y-4">
              {/* Invisible Honeypot input to trap auto-filling spam bots */}
              <input 
                type="text" 
                name="prostate_security_honey" 
                value={honeypotInput}
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-400">
                  Verify Human Identity: Solve the simple sum
                </label>
                <div className="flex gap-3 items-center">
                  <div className="bg-slate-950 border border-white/10 rounded-xl px-4 py-3 font-mono text-lg font-black text-blue-400 tracking-widest min-w-[100px] text-center">
                    {mathNumbers.n1} + {mathNumbers.n2} = ?
                  </div>
                  <input 
                    type="number" 
                    required
                    ...
                    className="flex-1 bg-slate-950 border border-white/10 focus:border-blue-500 rounded-xl px-4 py-3 font-bold text-white outline-none transition-all placeholder:text-slate-600 text-lg"
                  />
                </div>
              </div>

              {challengeErrorText && (
                <p className="text-xs text-red-400 font-bold bg-red-500/10 border border-red-500/20 p-3 rounded-lg flex items-center gap-2">
              ...
              <button 
                type="submit" 
                className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3.5 rounded-xl font-bold transition-all uppercase tracking-wider text-sm shadow-lg shadow-blue-600/10"
              >
                Release Secure Gateway Access
              </button>
            </form>
          </div>

          <div className="text-center text-[10px] text-slate-500 tracking-wider">
            SECURITY SEC_ID: COMP-USA-NODE-PROV • ENHANCED IP TARGET PROTECTION
          </div>
        </div>
      </div>
    );
  ...
  if (securityStatus === 'failed') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
        <div className="max-w-md w-full bg-white rounded-[40px] p-8 border border-slate-100 shadow-2xl space-y-6">
          <div className="text-center space-y-4">
            <div className="mx-auto w-16 h-16 bg-amber-50 text-amber-500 border border-amber-100 rounded-2xl flex items-center justify-center">
              <Fingerprint className="w-8 h-8" />
            </div>
            <div className="space-y-1">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-850 border border-amber-100 rounded-full text-[10px] font-bold uppercase tracking-wider">
                ⚠️ High Risk Robotic Profile Identified
              </span>
              <h1 className="text-2xl font-black text-brand-navy tracking-tight uppercase leading-none mt-2">
                System Verification Required
              </h1>
              <p className="text-xs font-semibold text-slate-500 leading-relaxed max-w-sm mx-auto">
                Client-side defenses detected automated execution scripts (Selenium logs, Webdrivers, or unmapped script coordinates) active in this browser.
              </p>
            </div>
          </div>

          <div className="bg-slate-50 border border-slate-100 rounded-3xl p-5 space-y-4">
            <h4 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">How to verify human standing:</h4>
            <ul className="text-xs text-slate-500 space-y-2 list-disc pl-4 leading-normal">
              <li>Formulate and submit the simple logic arithmetic challenge below.</li>
              <li>Automation profiling flags are flushed instantly on pass.</li>
            </ul>

            <form onSubmit={handleVerifySubmission} className="space-y-4 pt-3 border-t border-slate-100">
              {/* Invisible Honeypot input to trap auto-filling spam bots */}
              <input 
                type="text" 
                name="prostate_security_honey" 
              ...
              <div className="space-y-1">
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">
                  Prove Humanity Code Answer:
                </label>
                <div className="flex gap-2 items-center">
                  <div className="bg-white border border-slate-200/80 rounded-xl px-4 py-2.5 font-mono text-base font-black text-slate-700 min-w-[90px] text-center">
                    {mathNumbers.n1} + {mathNumbers.n2} = ?
                  </div>
                  <input 
                    type="number" 
                    ...
                    value={userMathAnswer}
                    onChange={(e) => setUserMathAnswer(e.target.value)}
                    className="flex-1 bg-white border border-slate-200 focus:border-blue-500 rounded-xl px-4 py-2.5 font-bold text-slate-800 outline-none transition-all text-base"
                  />
                </div>
              </div>

              {challengeErrorText && (
                <p className="text-xs text-red-500 font-bold bg-red-50/10 border border-red-500/10 p-2.5 rounded-lg flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 shrink-0" />
                  {challengeErrorText}
                </p>
              )}
              ...
              <button 
                type="submit" 
                className="w-full bg-brand-navy hover:bg-slate-800 text-white py-3 rounded-xl font-bold transition-all uppercase tracking-wider text-xs shadow-md"
              >
                Unblock human console session
              </button>
            </form>
          </div>

          <div className="text-center text-[9px] text-slate-400 font-mono tracking-wider">
            SECURITY METRIC PROFILE: LOCK_SEC_V9
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h1 className="text-4xl lg:text-7xl font-serif font-black leading-tight mb-8">
              Reclaim Your <span className="text-blue-400">Vitality</span> & Sleep Through The Night
            </h1>
